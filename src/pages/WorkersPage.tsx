  ...
  return (
    <PageTransition>
      <div style={{ maxWidth: '1024px', margin: '0 auto', padding: '2rem 2rem' }} className="space-y-6">
        ...
        {active.length > 0 && (
          <div>
            <h2 className="text-sm font-medium text-neon-blue mb-3 uppercase tracking-wider">Active ({active.length})</h2>
        ...
        <div>
          <h2 className="text-sm font-medium text-gray-500 mb-3 uppercase tracking-wider">History ({inactive.length})</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {inactive.map((w, i) => (
          ...
          {!workers && <div className="text-center py-16 text-gray-500">Loading workers...</div>}
          {workers && workers.length === 0 && <div className="text-center py-16 text-gray-500">No workers yet.</div>}
        </div>
      </div>
    </PageTransition>
  )
...
function WorkerCard({ worker: w, onClick }: { worker: Worker; onClick: () => void }) {
  ...
  return (
    <GlassCard glowColor={glowColor} className="p-4 cursor-pointer" >
      <div onClick={onClick}>
